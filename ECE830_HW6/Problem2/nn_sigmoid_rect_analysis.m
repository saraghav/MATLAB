% Autoencoder neural network with sigmoid rectification
clear; close all; clc;
...
% k_alpha_max = 225, k_alpha_min = 25
k_alpha_list = [1 10 25 100 125 150 200 225 350];
rep_list = 1:10;
...
for index = 1:length(k_alpha_list)
    ...
    for rep = rep_list
        ...
        for iter=1:iter_max
            ...
            x = training_data(i_t,:)';
            x_i_t = [ x ; 1 ];
            h_i_t = [ sigmoid(W_km1' * x_i_t) ; 1 ];
            xhat_i_t = sigmoid(V_km1' * h_i_t);

            delta = (xhat_i_t - x) .* xhat_i_t .* (1 - xhat_i_t);
            eta = h_i_t .* (1 - h_i_t);
            V_mod = bsxfun(@times, eta, V_km1);
            V_prime_mod = V_mod(1:end-1,:); % exclude last row; constant term weights
            gamma = V_prime_mod * delta;
        ...
        for example=1:n_testing_examples
            ...
            x_nn = [ x ; 1 ];
            h_nn = [ sigmoid(W_nn' * x_nn) ; 1 ];
            xhat_nn = sigmoid(V_nn' * h_nn);
