function [frf_mag, frf_phase, frf_freq] = get_frf(sig_num, sig_den, time, CTorDS)
...
f_range = 800;   % +/- frequency range ONLY used for plotting
...
if CTorDS == 0
    % command tracking
    semilogx(f,abs(FRF),plot_spec); xlim([0.1 f_range]); hold on; grid on;    
elseif CTorDS == 1
    % dynamic stiffness
    loglog(f,abs(FRF),plot_spec); xlim([0.1 f_range]); hold on; grid on;
end
...
figure(p); p = p+1;
semilogx(f,(angle(FRF))*180/pi,plot_spec); xlim([0.1 f_range]); hold on; grid on;
frf_phase = rad2deg(angle(FRF));

figure(p); p = p+1;
semilogx(f,COH,plot_spec); ylim([0 1]); xlim([0.1 f_range]); hold on; grid on;