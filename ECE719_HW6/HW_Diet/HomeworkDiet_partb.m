...
% gamma_list = logspace(-2,0,25);
% gamma_list = linspace(1e-2,1,100);
gamma_list = 1:1:60;

for i=1:length(gamma_list)
    gamma_used(i) = gamma_list(i);
    
    Aeq = [
        vitamin_B1' + vitamin_B2' - 1/gamma_list(i) * fat' ;
    ];
    ...
    optimal_solutions{i} = x;
    optimal_cost(i) = fval;
    solution_type(i) = exitflag;
end

%% Output

feasible_gamma_used = gamma_used(solution_type==1);
feasible_optimal_cost = optimal_cost(solution_type==1);

optimal_cost_without_eq_constraint = 1.439738 * ones(size(feasible_gamma_used));

figure(1); hold on; grid on; grid minor; legend_list = {};
plot(feasible_gamma_used, feasible_optimal_cost, 'o-', 'LineWidth', 2); legend_list = [legend_list 'variation of optimal cost with $\gamma$'];
plot(feasible_gamma_used, optimal_cost_without_eq_constraint, '--', 'LineWidth', 2); legend_list = [legend_list 'optimal cost without equality constraint'];
legend(legend_list{:});